...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['complete_payment'])) {
    ...
    if ($connect) {
        ...
        $sql = "UPDATE registration 
                SET trans_id = '$txn_esc', payment_status = 'completed', pdate = '$pdate' 
                WHERE md5(id) = '$token_esc'";
        if (mysqli_query($connect, $sql)) {
            header("Location: payment-success.php?token=" . $token);
            exit;
        } else {
            $error = "Payment update failed: " . mysqli_error($connect);
        }
    } else {
        // Mock session database update
        foreach ($_SESSION['mock_db'] as $id => $row) {
            if (md5($row['id']) === $token) {
                $_SESSION['mock_db'][$id]['trans_id'] = $txn_id;
                $_SESSION['mock_db'][$id]['payment_status'] = 'completed';
                $_SESSION['mock_db'][$id]['pdate'] = $pdate;
                break;
            }
        }
        header("Location: payment-success.php?token=" . $token);
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/60 shadow-xl">
          ...
          <div class="space-y-4 text-sm">
            <div>
              ...
              <span class="text-slate-850 font-semibold text-base"><?= htmlspecialchars($ires['first_name'] ?? '') ?></span>
            </div>
            <div>
              <span class="text-slate-550 block text-xs">Category</span>
              <span class="text-slate-700 font-medium">Miss Universe 2026 Audition</span>
            </div>
