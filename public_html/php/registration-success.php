...
// Redirect back to registration if no token or record found
if (!$ires) {
...
$wa_msg .= "• Name: " . ($ires['first_name'] ?? '') . "\n";
$wa_msg .= "• Category: " . $category . "\n";
$wa_msg .= "• Phone: " . ($ires['mobile'] ?? '') . "\n";
$wa_msg .= "• Email: " . ($ires['email'] ?? '') . "\n";
$wa_msg .= "• City: " . ($ires['cityfsia'] ?? '') . "\n\n";
...
<body class="min-h-screen flex flex-col font-sans bg-slate-50">
  ...
  <!-- MAIN CONTAINER -->
  <main class="flex-grow flex items-center justify-center py-12 px-4 bg-slate-50">
    <div class="w-full max-w-2xl z-10">
      ...
      <div class="bg-white rounded-3xl shadow-xl p-8 md:p-10 border border-slate-200/60">
        ...
        <div class="bg-slate-50 rounded-2xl border border-slate-200/60 p-6 mb-8">
          ...
          <dl class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 text-sm">
            <div>
              ...
              <dd class="text-slate-800 font-semibold text-base"><?= htmlspecialchars($ires['first_name'] ?? '') ?></dd>
            </div>
            <div>
              <dt class="text-slate-500 font-medium mb-1">Track Category</dt>
              <dd class="text-amber-600 font-semibold text-base">
                <?= ($ires['regtype'] == '30') ? 'Miss Universe 2026' : 'Forever Star India Event' ?>
              </dd>
            </div>
            <div class="border-t border-slate-200/60 pt-3 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <dd class="text-slate-700 font-mono"><?= htmlspecialchars($ires['mobile'] ?? '') ?></dd>
                <dd class="text-slate-500 text-xs mt-0.5 truncate"><?= htmlspecialchars($ires['email'] ?? '') ?></dd>
              </div>
              <div>
                <dt class="text-slate-500 font-medium mb-1">Location Profile</dt>
                <dd class="text-slate-700 font-semibold"><?= htmlspecialchars($ires['cityfsia'] ?? '') ?></dd>
                <dd class="text-slate-500 text-xs mt-0.5"><?= htmlspecialchars($ires['state'] ?? '') ?></dd>
              </div>
            </div>
          </dl>
        </div>
        ...
        <div class="space-y-4">
          <!-- Primary CTA Button -->
          <a href="payment-new.php?token=<?= htmlspecialchars($token) ?>" 
             class="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-amber-500/20 transform hover:-translate-y-0.5 transition duration-150 flex items-center justify-center space-x-2 text-lg">
