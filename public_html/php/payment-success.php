...
$wa_cert_msg = "Hello Support, I have successfully completed my payment for Miss Universe 2026.\n\n";
$wa_cert_msg .= "• Registration ID: " . ($ires['id'] ?? '') . "\n";
$wa_cert_msg .= "• Transaction ID: " . ($ires['trans_id'] ?? '') . "\n";
$wa_cert_msg .= "• Name: " . ($ires['first_name'] ?? '') . "\n\n";
        ...
        <div class="bg-slate-50 rounded-2xl border border-slate-200/60 p-6 mb-8 space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm font-mono">
            <div class="flex flex-col">
              ...
              <span class="text-slate-800 font-semibold mt-1 break-all select-all"><?= htmlspecialchars($ires['trans_id'] ?? '') ?></span>
            </div>
            
            <div class="flex flex-col">
              <span class="text-slate-400 text-xs uppercase">Amount Processed</span>
              <span class="text-amber-600 font-bold text-base mt-0.5"><?= htmlspecialchars($meta_tag['pay'] ?? '₹2,999') ?></span>
            </div>

            <div class="flex flex-col pt-2 border-t border-slate-200/40">
              <span class="text-slate-400 text-xs uppercase">Timestamp</span>
              <span class="text-slate-700 mt-1"><?= htmlspecialchars($ires['pdate'] ?? date('Y-m-d H:i:s')) ?></span>
            </div>
