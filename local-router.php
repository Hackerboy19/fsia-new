...
if (strpos($uri, '/assets/') === 0) {
    ...
    // Check in public_html first (where assets actually live)
    $filePath = __DIR__ . '/public_html' . $newUri;
    ...
    if (file_exists($filePath) && !is_dir($filePath)) {
        ...
        elseif ($ext === 'js') $mimeType = 'application/javascript';
        elseif ($ext === 'gif') $mimeType = 'image/gif';
        elseif ($ext === 'webp') $mimeType = 'image/webp';
        elseif ($ext === 'svg') $mimeType = 'image/svg+xml';
...
if (file_exists($publicHtmlPath) && !is_dir($publicHtmlPath)) {
    ...
    $ext = pathinfo($publicHtmlPath, PATHINFO_EXTENSION);
